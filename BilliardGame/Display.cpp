...
void DisplayBoard()
{
	...
	if (Freemode && isDisplayBox && Gamemode != SNOOKER)
	{
		...
		isDisplayBox = 0;
		MessageBox(NULL, str[turn], _T("Manager"), MB_OK | MB_SYSTEMMODAL);
	}
	...
	if (m.x >= startX + 50 && m.x <= startX + 145 && m.y >= startY + height + 5 && m.y <= startY + height + 35)
	{
		...
		{
			PlayQuitMidi();
			int answer = MessageBox(NULL, _T("是否保存？"), _T("Manager"), MB_YESNO | MB_ICONQUESTION | MB_SYSTEMMODAL);
...
void PowerControl()
{
	...
	while (isClickedMouse == 1 && finalhit == 0)
	{
		...
		if (Gamemode == POOL || Gamemode == NINEBALL)
		{
			outtextxy(755, 555, str);
			putimage(810, 555, &img_power);
			putimage(power_x, 552, &img_powercontrol);
		}
		else if (Gamemode == SNOOKER)
		{
			outtextxy(1145, 760, str);
			putimage(1200, 760, &img_power);
			putimage(power_x, 758, &img_powercontrol);
		}
		...
		while (MouseHit())
		{
			...
			if (Gamemode == POOL || Gamemode == NINEBALL)
			{
				if (m.x >= power_x && m.x <= power_x + 16 && m.y >= 550 && m.y <= 568 && m.uMsg == WM_LBUTTONDOWN)
					ctrl_power = 1;
			...
			{
				if (m.x >= power_x && m.x <= power_x + 16 && m.y >= 758 && m.y <= 758 + 19 && m.uMsg == WM_LBUTTONDOWN)
					ctrl_power = 1;
				if (m.uMsg == WM_LBUTTONUP)
					ctrl_power = 0;
			}
...
void load_img()
{
	...
	loadimage(&img_power, _T("Source/power.jpg"));
	loadimage(&img_powercontrol, _T("Source/powercontrol.jpg"));
	loadimage(&img_introduce, _T("Source/介绍.png"));
}
...
void UpdateWithInput_menu()
{
	...
	{
		m = GetMouseMsg();
		if ((m.x - 565) * (m.x - 565) + (m.y - 515) * (m.y - 515) <= 65 * 65)
		{
			isRoll = 0;
		}
		else {
		...
		if (m.x >= 315 && m.x <= 440 && m.y <= 650 && m.y >= 536)
		{
			...
			{
				PlayMenuAMidi();
				int answer = MessageBox(NULL, _T("是否退出？"), _T("Manager"), MB_YESNO | MB_ICONQUESTION | MB_SYSTEMMODAL);
...
void DisplaySetting()
{
	...
	const TCHAR* soundStr[2] = { _T("静音"),_T("正常") };
	PlaySettingMusic();
	...
	while (1)
	{
		...
		else if (m.x >= 300 && m.x <= 400 && m.y >= 95 && m.y <= 130)
		{
			setlinecolor(BLACK);
			rectangle(300 - 2, 95 - 2, 400 + 2, 130 + 2);
			if (m.uMsg == WM_LBUTTONDOWN)
			{
				isSound = !isSound;
				PlaySettingMusic();
		...
		else if (m.x >= 300 && m.x <= 400 && m.y >= 145 && m.y <= 180)
		{
			setlinecolor(BLACK);
		...
		else if (m.x >= 300 && m.x <= 400 && m.y >= 195 && m.y <= 230)
		{
			setlinecolor(BLACK);
		...
		else if (m.x >= 300 && m.x <= 400 && m.y >= 245 && m.y <= 280)
		{
			setlinecolor(BLACK);
			...
			if (m.uMsg == WM_LBUTTONDOWN)
			{
				...
				{
					MessageBox(NULL, _T("清除成功！"), _T("Manager"), MB_OK | MB_SYSTEMMODAL);
				}
				else {
					USES_CONVERSION;
					char s[1000];
					sprintf(s, "Error: %s", strerror(errno));
					MessageBox(NULL, A2W(s), _T("错误"), MB_OK | MB_ICONWARNING | MB_SYSTEMMODAL);
				}
			}
		}
