	...
	if (MouseHit())
	{
		...
		if (Gamemode == SNOOKER)
		{
			if (isChosenBall)
			{
				...
				if (!isMoved)
				{
					if (m.uMsg == WM_LBUTTONUP)
					{
						if (isStart)
						{
							if ((m.x - kitchenLoaction.X) * (m.x - kitchenLoaction.X) + (m.y - kitchenLoaction.Y) * (m.y - kitchenLoaction.Y) <= boardHeight[Gamemode] * boardHeight[Gamemode] / 36.0 && m.x - kitchenLoaction.X >= 0)
							{
								...
								MessageBox(NULL, _T("请点击在D型区域内部!"), _T("Manager"), MB_OK | MB_SYSTEMMODAL);
								isClickedMouse = 0;
							}
						}
						else {
			...
			if (m.uMsg == WM_LBUTTONUP)
			{
				...
				if (m.x >= startX + 50 - 2 && m.x <= startX + 145 + 2 && m.y >= startY + height + 5 - 2 && m.y <= startY + height + 35 + 2)
				{
					...
					if (Freemode == 0)
					{
						...
						if (m.x <= boardWidth[Gamemode] && m.x >= startX && m.y >= startY && m.y <= boardHeight[Gamemode])
						{
							ResetLocation(0, m.x, m.y);
							Freemode = 0;
						}
						else {
							MessageBox(NULL, _T("请点击在桌子内部!"), _T("Manager"), MB_OK | MB_SYSTEMMODAL);
							Freemode = 1;
						}
					}
				}
...
int Enterhole()
{
	...
	for (bBallNode* temp = ballHead; temp != NULL; temp = temp->next)
	{
		...
		if (sqrt(pow(temp->bBall.ballLocation.X - mx, 2) + pow(temp->bBall.ballLocation.Y - my, 2)) <= RADIUS_HOLE * 1.3) {
...
void showwinner()
{
	...
	MessageBox(NULL, winnerstr[winner], _T("Manager"), MB_OK | MB_SYSTEMMODAL);
}
	...
	if (number != -1)
	{
		...
		else if (number >= 16 && number <= 21)
		{
			...
			if (hitStripBall == 1 && firstHitID < 16)//该打彩球进洞时第一球打到了红球
				foul = 1;
			if (hitStripBall == 1 && number != objectBallID)//把其他彩球打进洞
				foul = 1;
...
{
	int i = 0;
	if (isClickedMouse == 0 && addgametime == 1) {
		if (isenterthehole == 0)
		{
			turn = !turn;
			if (hitStripBall == 0 && firstHitID >= 16)
			{
				...
				MessageBox(NULL, _T("非法击球1！"), _T("Reminder"), MB_OK | MB_SYSTEMMODAL);
			}
			if (hitStripBall == 1 && firstHitID != objectBallID && Allredinholes() != 15)
			{
				hitStripBall = 0;
				if (firstHitID > objectBallID)
					score[turn] += 4 > firstHitID - 14 ? 4 : firstHitID - 14;
				...
				MessageBox(NULL, _T("非法击球2！"), _T("Reminder"), MB_OK | MB_SYSTEMMODAL);
			}
			if (hitStripBall == 1 && Allredinholes() != 15)
				hitStripBall = 0;
			if (hitStripBall == 1 && firstHitID != objectBallID && Allredinholes() == 15)
			{
				if (firstHitID > objectBallID)
					...
					score[turn] += 4 > objectBallID - 14 ? 4 : objectBallID - 14;
				MessageBox(NULL, _T("非法击球！"), _T("Reminder"), MB_OK | MB_SYSTEMMODAL);
			}
		}
		else if (isenterthehole == 1)
		{
			if (hitStripBall == 0 && foul == 0 && Allredinholes() != 15)
			{
				hitStripBall = 1;
				isChosenBall = 0;
				isRod = 0;
				MessageBox(NULL, _T("请点击选择下一杆要击打的彩球！"), _T("提示"), MB_OK | MB_SYSTEMMODAL);
				score[turn] += snookerpurecount;
			}
			else if (hitStripBall == 0 && foul == 1 && Allredinholes() != 15)
			{
				turn = !turn;
				isChosenBall = 1;
				if (firstHitID >= 16)
				{
					score[turn] += 4 > firstHitID - 14 ? 4 : firstHitID - 14;
					MessageBox(NULL, _T("击错目标球！"), _T("犯规"), MB_OK | MB_SYSTEMMODAL);
					for (bBallNode* temp = ballHead; temp != NULL; temp = temp->next)
					{
				...
				{
					MessageBox(NULL, _T("彩球进洞！"), _T("犯规"), MB_OK | MB_SYSTEMMODAL);
					for (bBallNode* temp = ballHead; temp != NULL; temp = temp->next)
					{
						if (temp->bBall.ID >= 16 && entertheball[temp->bBall.ID] == 1)
						{
			...
			else if (hitStripBall == 1 && foul == 0 && Allredinholes() != 15)
			{
				hitStripBall = 0;
				isChosenBall = 1;
				for (bBallNode* temp = ballHead; temp != NULL; temp = temp->next)
			...
			else if (hitStripBall == 1 && foul == 1 && Allredinholes() != 15)
			{
				hitStripBall = 0;
				turn = !turn;
				isChosenBall = 1;
				...
				{
					MessageBox(NULL, _T("目标球和其他球都进洞！"), _T("犯规"), MB_OK | MB_SYSTEMMODAL);
					for (bBallNode* temp = ballHead; temp != NULL; temp = temp->next)
					{
						if (temp->bBall.ID >= 16 && entertheball[temp->bBall.ID] == 1)
						{
				...
				{
					MessageBox(NULL, _T("非目标球都进洞！"), _T("犯规"), MB_OK | MB_SYSTEMMODAL);
					for (bBallNode* temp = ballHead; temp != NULL; temp = temp->next)
					{
						if (temp->bBall.ID >= 16 && entertheball[temp->bBall.ID] == 1)
						{
					...
					score[turn] += firstHitID - 14 > (4 > objectBallID - 14 ? 4 : objectBallID - 14) ? firstHitID - 14 : (4 > objectBallID - 14 ? 4 : objectBallID - 14);
				}
				objectBallID = 0;
			}
			else if (foul == 2)
			{
				...
				if (Allredinholes() != 15)
					...
					hitStripBall = 1;
				MessageBox(NULL, _T("白球进洞！"), _T("犯规"), MB_OK | MB_SYSTEMMODAL);
				for (bBallNode* temp = ballHead; temp != NULL; temp = temp->next)
				{
					if (temp->bBall.ID >= 16 && entertheball[temp->bBall.ID] == 1)
					{
			...
			if (Allredinholes() == 15)
			{
				...
				if (foul == 0)
				{
					score[turn] += objectBallID - 14;
					entertheball[objectBallID] = 2;
				}
				else
				{
					turn = !turn;
